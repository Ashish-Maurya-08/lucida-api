...
import Lucida from './index.js';
import fs from 'node:fs';
let testOptions;
if (!process.env.TEST_OPTIONS) {
    console.error('No test options found in environment variables. Trying to read from .test-options.json');
    try {
        const testOptionsFile = fs.readFileSync('.test-options.json', 'utf-8');
        testOptions = JSON.parse(testOptionsFile);
    }
    catch (error) {
        console.error('No test options found in environment variables or .test-options.json. Exiting...');
        process.exit(1);
    }
}
else
...
function start() {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const [module, options] of Object.entries(testOptions)) {
            ...
            const { default: Streamer } = yield import(`lucida/streamers/${module}`);
            lucidaOptions.modules[module] = new Streamer(options);
            if (username && password) {
                (_a = lucidaOptions.logins) !== null && _a !== void 0 ? _a : (lucidaOptions.logins = {});
                lucidaOptions.logins[module] = { username, password };
            }
        }
        const lucida = new Lucida(lucidaOptions);
        yield lucida.login(true);
        const testResults = {};
        for (const moduleName of Object.keys(testOptions)) {
            const module = lucida.modules[moduleName];
            console.log(`Testing ${moduleName}...`);
            (_b = testResults[moduleName]) !== null && _b !== void 0 ? _b : (testResults[moduleName] = {
            ...
            if (module.testData) {
                for (const url in module.testData) {
                    ...
                    if (!success.type) {
                        console.error(`Type mismatch for ${url} with the ${moduleName} streamer. Expected: ${expected.title}, got: ${actual.title}`);
                    }
                    if (!success.title) {
                        console.error(`Title mismatch for ${url} with the ${moduleName} streamer. Expected: ${expected.title}, got: ${actual.title}`);
                    }
        ...
        const readableResults = Object.entries(testResults).map((result) => {
            ...
            const score = Object.values(moduleResults).filter((result) => result === 1 || result != false).length;
            const percentage = (score / Object.keys(moduleResults).length) * 100;
            return `- ${moduleName}: ${percentage.toFixed(2)}%`;
        });
        fs.writeFileSync('.test-results.md', `## Testing complete\n\n${readableResults.join('\n')}\n\n<details><summary>Results</summary>\n\n\`\`\`json\n` +
